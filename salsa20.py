...
from ctypes import (
    cdll, c_char_p, c_int, c_uint64, create_string_buffer
)

# replacement for imp.find_module('_salsa20')[1]
spec = importlib.util.find_spec('_salsa20')
if spec is None or not spec.origin:
    raise ImportError('Could not find _salsa20 shared library')
_salsa20 = cdll.LoadLibrary(spec.origin)

_stream_salsa20 = _salsa20.exp_stream_salsa20
_stream_salsa20.argtypes = [c_char_p, c_uint64, c_char_p, c_char_p]
_stream_salsa20.restype = c_int

_stream_salsa20_xor = _salsa20.exp_stream_salsa20_xor
_stream_salsa20_xor.argtypes = [c_char_p, c_char_p, c_uint64, c_char_p, c_char_p]
_stream_salsa20_xor.restype = c_int

_stream_xsalsa20 = _salsa20.exp_stream_xsalsa20
_stream_xsalsa20.argtypes = [c_char_p, c_uint64, c_char_p, c_char_p]
_stream_xsalsa20.restype = c_int

_stream_xsalsa20_xor = _salsa20.exp_stream_xsalsa20_xor
_stream_xsalsa20_xor.argtypes = [c_char_p, c_char_p, c_uint64, c_char_p, c_char_p]
...
def Salsa20_keystream(length, nonce, key):
    ...
    if len(nonce) != 8: raise ValueError('invalid nonce length')
...
def Salsa20_xor(message, nonce, key):
    ...
    if len(key) != 32: raise ValueError('invalid key length')
    if len(nonce) != 8: raise ValueError('invalid nonce length')
    if len(message) <= 0: raise ValueError('invalid message length')
...
def XSalsa20_keystream(length, nonce, key):
    ...
    _ensure_bytes(key)
    if len(key) != 32: raise ValueError('invalid key length')
    if len(nonce) != 24: raise ValueError('invalid nonce length')
    if length <= 0: raise ValueError('invalid length')
    outbuf = create_string_buffer(length)
